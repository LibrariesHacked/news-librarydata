$(function () {
        ...
        },
        {
            ...
            render: function (data, type, row) {
                if (type === 'sort') {
                    ...
                    return (
                        '<a title="Go to Public Libraries News" target="_blank" href="http://www.publiclibrariesnews.com/' + data + '"><i class="fa fa-external-link-square" aria-hidden="true"></i></a>&nbsp;&nbsp;' +
                        '<a title="Share on Twitter" target="_blank" href="' + twitter_link + '"><i class="fa fa-twitter-square text-info" aria-hidden="true"></i></a>'
                    );
                }
            }
        },
    ...
    var setLine = function () {
        ...
        $.each(Object.keys(News.type_month_counts), function (i, type) {
            line.data.labels = $.map(Object.keys(News.type_month_counts[type]), function (d, i) { return moment(d, 'YYYYMM').format('MMM YY') });
            line.data.datasets.push({
                backgroundColor: [background_colours[i]],
                borderColor: [border_colours[i]], borderWidth: 1, label: type, data: Object.keys(News.type_month_counts[type]).map(function (mth, y) { return News.type_month_counts[type][mth]; }) });
        });
        line.update();
    };
